...
from miseventos.infrastructure.api.controllers.slot_controller import add_slot_controller, find_slot_by_event_id_controller, delete_slot_controller
...
def register_slotcase(db:Session=Depends(get_db)):
    repo = SlotImplement(db) # instancia de UserImplement con la sesión de la base de datos
...
async def register_slot(body: SlotRequest, usecase: SlotUseCase = Depends(register_slotcase)):
...
async def get_slot_by_event_id(event_id: UUID, usecase: SlotUseCase = Depends(register_slotcase)):
