...
from miseventos.infrastructure.persistence.postgresql.schemas.user_schema import UserRequest, UserEmailRequest
from miseventos.infrastructure.api.controllers.use_controller import add_user_controller, find_by_email_controller

def register_usecase(db:Session=Depends(get_db)):
    repo = UserImplement(db) # instancia de UserImplement con la sesión de la base de datos
...
async def register_user(body: UserRequest, usecase: UserUseCase = Depends(register_usecase)):
    response =  add_user_controller(usecase)

    return await response(body)

@user_router.get("/user/{email}")
async def get_user_by_email(email: str, usecase: UserUseCase = Depends(register_usecase)):
