...
from miseventos.repositories.session_register_repository import SessionRegisterRepository
...
class SessionRegisterImplement(SessionRegisterRepository):
    ...
    def add_session_register(self, body: SessionRegisterRequest) -> registerResponse:
        try:
            ...
            event_registration = self.session.query(EventRegistration).filter(
                and_(
            ...
            session_reg_count = self.session.query(SessionRegistration).filter(
                SessionRegistration.session_id == body.session_id
            ).count()

            event_reg_count = self.session.query(EventRegistration).filter(
                EventRegistration.event_id == body.event_id
            ).count()
            ...
            print(f"ID de registro de sesión: {session_registration_id}")
            print(f"Usuarios en sesión: {session_reg_count}")
            print(f"Usuarios en evento: {event_reg_count}")
            print(f"Session ID: {body.session_id}")
            print(f"Event ID: {body.event_id}")
    ...
    def delete_session_register(self, body: SessionRegisterDeleteRequest) -> SessionDeleteResponse:
        try:
            session_registration = self.session.query(SessionRegistration).filter(
                and_(
                    SessionRegistration.user_id == body.user_id,
            ...
            return SessionDeleteResponse(
                success=False,
                message=f"Error interno al eliminar el registro"
            )