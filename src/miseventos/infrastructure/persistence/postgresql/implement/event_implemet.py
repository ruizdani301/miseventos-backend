...
from miseventos.infrastructure.persistence.postgresql.models.event_model import Event as EventModel
...
class EventImplement(EventRepository):
    ...
    def get_events_paginated(self, page: int = 1, limit: int = 10) -> List[EventEntity] | None:
    
        offset = (page - 1) * limit
        
        event_models = self.session.query(EventModel)\
            .offset(offset)\
            .limit(limit)\
            .all()
