...
from miseventos.infrastructure.persistence.postgresql.models.time_model import TimeSlot as TimeSlotModel
from miseventos.infrastructure.persistence.postgresql.schemas.schema import Response
from uuid import UUID
from typing import List
from miseventos.infrastructure.persistence.postgresql.schemas.slot_schema import SlotDeleteResponse
...
class SlotImplement(SlotRepository):
    ...
    def add_slot(self, slot: TimeSlotEntity) -> TimeSlotEntity | None:
        ...
        existing_slots = self.session.query(TimeSlotModel).filter(
            and_(
    ...
    def get_slot_by_event_id(self, event_id: UUID)-> List[TimeSlotEntity] | None:
        ...
        slot_models = self.session.query(TimeSlotModel)\
    ...
    def delete_slot(self, slot_id: UUID)->Response:
        slot_models = self.session.query(TimeSlotModel)\
            .filter(TimeSlotModel.id == slot_id)
