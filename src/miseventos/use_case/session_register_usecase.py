...
class SessionRegisterUseCase:
    ...
    def add_session_register(self, session: SessionRegisterRequest) -> registerResponse:
        ...
        if not new_register:
            return SessionRegisterResponse(success=False, error_message="Error saving register.")
        if not new_register.success:
            return SessionRegisterResponse(success=False, error_message=new_register.error_message)
        
        return SessionRegisterResponse(success=True, error_message=None, session_detail=new_register)
